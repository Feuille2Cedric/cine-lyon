...
let data,cinemas=new Map(),selected=new Set(),date=parisDate(),view='week',query='',version='',hourHeight=88,overflowGroups=[],renderTimer;
function save(){try{localStorage.setItem('travelling-settings',JSON.stringify({selected:[...selected],view,hourHeight}));}catch{}}
...
function visibleEvents(){const {start,end}=period();return data.sessions.filter(s=>s.start.slice(0,10)>=start&&s.start.slice(0,10)<end&&selected.has(s.cinema)&&normalize(s.title).includes(normalize(query))&&(!version||s.version.startsWith(version)));}
...
function details(id){
  ...
  $('#detail-content').innerHTML=`<div class="detail-cinema" style="${palette(c)}"><span class="detail-dot"></span>${esc(c.name)}</div><h2 id="detail-title">${esc(s.title)}</h2><div class="detail-meta"><div><span>Le rendez-vous</span><strong>${esc(fmt(s.start.slice(0,10),{weekday:'long',day:'numeric',month:'long'}))}</strong></div><div><span>La séance</span><strong>${clock(minute)}${s.duration?' → '+clock(minute+s.duration):''} ${esc(s.version)}</strong></div><div><span>Le cinéma</span><strong>${esc(c.address)}</strong></div><div><span>Durée du film</span><strong>${s.duration?Math.floor(s.duration/60)+' h '+String(s.duration%60).padStart(2,'0'):'Non communiquée'}</strong></div></div>${s.extra?`<p>${esc(s.extra)}</p>`:''}<p>${s.duration?'L’heure de fin est estimée, hors publicités, présentations et échanges.':'Durée non communiquée : le bloc occupe 90 minutes à titre indicatif.'}</p><div class="detail-actions"><a class="primary" href="${esc(safeUrl(s.url))}" target="_blank" rel="noopener noreferrer">Voir / réserver la séance ↗</a><button id="export-ics">Ajouter à mon agenda</button></div>`;
...
$('#previous').onclick=()=>{date=addDays(date,view==='day'?-1:-7);render();};$('#next').onclick=()=>{date=addDays(date,view==='day'?1:7);render();};$('#today').onclick=()=>{date=parisDate();render();};
...
$('#search').oninput=e=>{query=e.target.value;clearTimeout(renderTimer);renderTimer=setTimeout(render,100);};$('#version').onchange=e=>{version=e.target.value;render();};$('#zoom').oninput=e=>{hourHeight=Number(e.target.value);render();};$('#reset').onclick=()=>{selected=new Set(cinemas.keys());query='';version='';$('#search').value='';$('#version').value='';render();};
...
try{const response=await fetch('./data/schedule.json',{cache:'no-cache'});if(!response.ok)throw new Error('HTTP '+response.status);data=await response.json();cinemas=new Map(data.cinemas.map(c=>[c.id,c]));selected=new Set(cinemas.keys());
  try{const saved=JSON.parse(localStorage.getItem('travelling-settings'));if(saved){if(Array.isArray(saved.selected))selected=new Set(saved.selected.filter(id=>cinemas.has(id)));if(['week','day','list'].includes(saved.view))view=saved.view;if(saved.hourHeight>=60&&saved.hourHeight<=160)hourHeight=saved.hourHeight;}}catch{}
  $('#zoom').value=hourHeight;renderSources();render();setInterval(()=>{if(document.visibilityState==='visible'&&!$('#details').open)render();},60000);
